...
def main():
    ...
    for j in range(len(indices)):
        ...
        if not ok:break
        row=pred[j]
        # Draw keypoints on full frame (solid dark backing, solid colour on top — no AA to avoid ring artefacts)
        for i,(x,y,lik) in enumerate(row):
            if np.isfinite(x) and np.isfinite(y) and lik>=threshold:
                cv2.circle(frame,(int(x),int(y)),dot_r+2,(0,0,0),-1)
                cv2.circle(frame,(int(x),int(y)),dot_r,colors[i],-1)
        ...
        for i,(x,y,lik) in enumerate(row):
            if np.isfinite(x) and np.isfinite(y) and lik>=threshold:
                ...
                if 0<=lx<cw and 0<=ly<ch:
                    ...
                    cv2.circle(zoom,(px,py),zoom_r+2,(0,0,0),-1)
                    cv2.circle(zoom,(px,py),zoom_r,colors[i],-1)
