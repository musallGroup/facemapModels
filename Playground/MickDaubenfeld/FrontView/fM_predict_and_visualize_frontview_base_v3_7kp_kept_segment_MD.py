"""
predict_and_visualize_frontview_base_v3_7kp_kept_segment.py

Runs Frontview_Base_v3_7kp_KeptTongue.pt on a selected short Cam0 segment and saves:
- HDF5 predictions
- JSON metadata
- MP4 QC overlay with stable playback speed
- PNG image sequence for exact frame-by-frame inspection

Default start: 14 minutes
...
def ask_segment_settings(
    ...
    if start_minutes is None:
        root.destroy()
        raise RuntimeError(
            "Segment selection was cancelled."
        )
    ...
    duration_seconds = simpledialog.askfloat(
        ...
        maxvalue=max(
            1.0,
            video_duration_seconds - start_minutes * 60.0,
        ),
        parent=root,
    )
    ...
    if duration_seconds is None:
        raise RuntimeError(
            "Segment duration selection was cancelled."
        )
...
def inspect_model_file() -> None:
    ...
    for head_name in required_heads:
        if head_name not in state_dict:
            raise KeyError(
                f"Missing model head:\n{head_name}"
            )

        shape = tuple(
            state_dict[head_name].shape
        )
...
def read_video_information(
    ...
    if fps <= 0 or total_frames <= 0:
        raise RuntimeError(
            "The video reports invalid metadata."
        )
...
def create_frame_indices(
    ...
    if start_frame >= total_frames:
        raise RuntimeError(
            "The selected start time lies outside the video."
        )
    ...
    end_frame_exclusive = min(
        start_frame + requested_frames,
        total_frames,
    )

    if end_frame_exclusive <= start_frame:
        raise RuntimeError(
            "The selected segment contains no frames."
        )
...
def save_predictions_h5(
    ...
    with h5py.File(
        ...
        h5_file.attrs["video_path"] = str(
            video_path
        )
        h5_file.attrs["model_path"] = str(
            MODEL_PATH
        )
        h5_file.attrs["number_of_keypoints"] = len(
            KEYPOINT_NAMES
        )
...
def create_qc_outputs(
    ...
    print("\n========================================")
    print("CREATING QC VIDEO AND FRAME IMAGES")
    print("========================================")
    print(
        f"QC playback FPS: {QC_PLAYBACK_FPS:.1f}"
    )
    print(
        f"Save PNG frames: {SAVE_FRAME_IMAGES}"
    )

    if SAVE_FRAME_IMAGES:
        print(
            f"Frame folder: {frames_output_dir}"
        )
        ...
        for prediction_index, source_frame_index in enumerate(
            ...
            if SAVE_FRAME_IMAGES:
                frame_output_path = (
                    frames_output_dir
                    / (
                        f"segment_{prediction_index:06d}_"
                        f"sourceframe_{source_frame_index:09d}.png"
                    )
                )
...
def main() -> None:
    print("\n========================================")
    print("FRONTVIEW BASE V3 KEPT-TONGUE — SEGMENT QC")
    print("7 KEYPOINTS")
    print("========================================")
    ...
    print(
        f"\nVideo duration: "
        f"{video_duration_seconds / 60.0:.2f} min"
    )

    (
        start_minutes,
        duration_seconds,
    ) = ask_segment_settings(
        video_duration_seconds
    )
    ...
    actual_start_seconds = (
        frame_indices[0] / source_fps
    )
    actual_end_seconds = (
        frame_indices[-1] / source_fps
    )
    ...
    print(
        f"Start time:       "
        f"{actual_start_seconds / 60.0:.3f} min"
    )
    print(
        f"End time:         "
        f"{actual_end_seconds / 60.0:.3f} min"
    )
    print(
        f"Duration:         "
        f"{len(frame_indices) / source_fps:.2f} s"
    )
    print(
        f"Frames processed: "
        f"{len(frame_indices)}"
    ...
    video_output_path = (
        OUTPUT_DIR
        / f"{output_stem}_QC_30fps.mp4"
    )

    frames_output_dir = (
        OUTPUT_DIR
        / f"{output_stem}_QC_frames"
    )
