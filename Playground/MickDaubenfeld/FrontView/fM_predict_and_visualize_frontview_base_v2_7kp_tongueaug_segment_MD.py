"""
predict_and_visualize_frontview_base_v2_7kp_tongueaug_segment.py

Runs Frontview_Base_v2_7kp_TongueAug.pt on a selected short Cam0 segment and saves:
- HDF5 predictions
- JSON metadata
...
MODEL_PATH = PROJECT_ROOT / "Current_Model" / "Frontview_Base_v2_7kp_TongueAug.pt"
...
def create_qc_outputs(
    ...
    print("========================================")
    print(f"QC playback FPS: {QC_PLAYBACK_FPS:.1f}")
    print(f"Save PNG frames: {SAVE_FRAME_IMAGES}")
    if SAVE_FRAME_IMAGES:
        print(f"Frame folder: {frames_output_dir}")

    # Seek only once to the first requested source frame.
    # All following frames are then read sequentially. This is faster and
    # avoids the jerky behavior caused by seeking for every single frame.
    ...
    try:
        for prediction_index, source_frame_index in enumerate(frame_indices):
            ...
            if not success:
                raise RuntimeError(
                    f"Could not read source frame {source_frame_index}."
                )
...
def main() -> None:
    print("\n========================================")
    print("FRONTVIEW BASE V2 TONGUE-AUGMENTED — SEGMENT QC")
    print("7 KEYPOINTS")
    print("========================================")
    ...
    video_output_path = OUTPUT_DIR / f"{output_stem}_QC_30fps.mp4"
    frames_output_dir = OUTPUT_DIR / f"{output_stem}_QC_frames"
